...
| 最後修改起訖日 | 後端 `GetSOPList_Edit`（2026-09-18 起，見下） |
| 快速搜尋 | 後端 `GetSOPList_Edit`（2026-09-18 起，見下） |

2026-09-18 之前 `startDate` / `endDate` 雖然收但後端那段過濾邏輯**整段被註解掉了**
（2026-06-08 改成 order by 客戶別時停用），前端改成自己對「最後修改時間」過濾；
快速搜尋當時也是純前端在目前結果內找。為了做後端分頁，這兩個條件改回後端做——
分頁的基準必須跟篩選條件一致，「後端只回第 2 頁」跟「前端只篩目前這批結果」是互斥的，
兩個同時開會讓使用者看到的筆數對不起來。

## 分頁（2026-09-18 起）

`GetSOPList_Edit` 新增 `keyword`、`tab`（`ongoing`/`finished`/`all`）、
`pageIndex`（0 起算）、`pageSize`（`<= 0` 代表不分頁）。查詢邏輯本身沒變
（`GetSopListCore` 仍是全撈進記憶體、LINQ to Objects 兜資料——見上面「與 1.0 的行為差異」），
分頁只是在最後把篩選＋權限過濾完的清單 `Skip().Take()` 再回傳，**沒有把查詢改寫成
SQL 層級的分頁**，DB 負載沒有變小，只是網路傳輸量與瀏覽器記憶體用量變小。

三個狀態頁籤（進行中/結案/全部）的筆數放在 `body2`（`SalesIssueListSummary`：
`totalCount`/`ongoingCount`/`finishedCount`/`allCount`），是「篩選後、切頁籤前」算出來的，
不會因為目前選哪個頁籤而變動；`totalCount` 才是目前頁籤篩選後的筆數，前端拿來算分頁列的頁數。

前端 `issues/index.vue` 的表頭排序（點欄位標題）**只在目前這一頁的資料內排序**，
不是全體資料排序——後端已經把資料切頁，前端拿不到其他頁的內容可以排。
要做到全體排序得讓後端也收排序欄位，目前沒做。
