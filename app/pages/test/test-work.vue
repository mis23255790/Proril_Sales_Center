...
const isWaterLeakage = ref(false)
...
const openTest = async (row: WoByPoItem) => {
  isWaterLeakage.value = row.passFlag !== '0'
...
const submitWaterLeakage = async () => {
  if (wlResult.value !== 'Y') {
    toast.add({ title: '不允許測試結果不合格', description: '此畫面僅可送出合格結果，不合格請改由覆核流程處理', color: 'warning' })
    return
  }
  testSubmitting.value = true
  try {
    const res = await apiFetch<ApiResponse<any>>('/TestApi/SaveTestDataWL', {
      params: { woNo: currentWoNo.value, productNo: currentProductNo.value, testPlace: testPlace.value, passFlag: wlResult.value }
    })
    if (res?.isSuccess) {
      toast.add({ title: '測試資料已送出', color: 'success' })
      await goToNextOrClose()
...
<template>
  <div>
    ...
    <UBreadcrumb :items="[{ label: '首頁', to: '/', icon: 'i-lucide-house' }, { label: '測試系統', icon: 'i-lucide-flask-conical' }, { label: '測試作業' }, { label: '測試作業' }]" class="mb-4" />
    ...
    <USlideover v-model:open="testOpen" :title="isWaterLeakage ? '著托測試' : '測試作業'" :description="header?.modelName">
      <template #body>
        ...
        <div v-else class="space-y-6">
          ...
          <UFormField v-else label="測試結果">
            <USelectMenu v-model="wlResult" value-key="value" :items="[{ label: '合格', value: 'Y' }, { label: '不合格', value: 'N' }]" class="w-full" />
          </UFormField>

          <USeparator />
          <TestMemoPanel v-if="currentWoNo" :link-type="isWaterLeakage ? 211 : 5" :link-number="currentWoNo" />
        </div>
      </template>
      <template #footer>
        <UButton :loading="testSubmitting" @click="isWaterLeakage ? submitWaterLeakage() : submitNormal()">
          送出
        </UButton>
      </template>
    </USlideover>
